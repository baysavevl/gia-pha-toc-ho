  ...
  return (
    <div className="min-h-screen bg-neutral">
      ...
      <main className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        <Link href="/bai-viet" className="text-sm font-bold text-amber-700 hover:text-amber-800">
          ← Quay lại Bài viết
        </Link>
        <article className="mt-8 overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm">
          ...
          <div className="p-6 sm:p-10">
            <div className="flex flex-wrap gap-2 text-xs font-bold uppercase tracking-[0.14em] text-amber-700">
              <span>{entry.category ?? "Bài viết"}</span>
              {entry.publishedAt && <span>• {formatVietnameseDate(entry.publishedAt)}</span>}
            </div>
