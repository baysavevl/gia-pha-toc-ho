...
export async function exportData(
  ...
  if (!isAdmin) {
    return { error: "Từ chối truy cập. Chỉ Ban trị sự mới có quyền này." };
  }
  ...
  let exportPersons = (allPersons ?? []) as unknown as PersonExport[];
  let exportRels = (allRels ?? []) as unknown as RelationshipExport[];
  let exportPrivateDetails = (allPrivateDetails ??
    []) as unknown as PersonDetailsPrivateExport[];
